using BeamCalcClient.Extensions;

namespace BeamCalcClient.Extensions
{
    /// <summary>
    ...
    public static class EnumerableExtensions
    {
        ...
        {
            return Math.Round(values.Min(p => Math.Min(property1(p), property2(p) ?? double.MaxValue)), precision);
        }
        ...
        {
            return Math.Round(values.Max(p => Math.Max(property1(p), property2(p) ?? double.MinValue)), precision);
        }
    }
}
